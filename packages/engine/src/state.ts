...
export type BlockState =
  ...
  | { type: "torch"; pos: VecObj; attachedFace: VecObj; lit: boolean; scheduledStateChange: number | null; burnedOut: boolean }
...
export class EngineState {
  ...
  constructor(data: { tickCounter: number; blocks: BlockState[]; events: { tick: number; positions: string[] }[] }) {
  ...
  static decode(encoded: string): EngineState {
    const json = atob(encoded)
    return new EngineState(JSON.parse(json))
  }

  toUrl(baseUrl: string): string {
    const url = new URL(baseUrl)
    url.searchParams.set("state", this.encode())
    return url.toString()
  }

  static fromUrl(url: string): EngineState | null {
    const parsed = new URL(url)
    const encoded = parsed.searchParams.get("state")
    if (!encoded) return null
    return EngineState.decode(encoded)
  }
}
...
export function serializeBlock(block: Block): BlockState {
  ...
  switch (block.type) {
    ...
    case "torch":
      return { type: "torch", pos, attachedFace: block.attachedFace.toJSON(), lit: block.lit, scheduledStateChange: block.scheduledStateChange, burnedOut: block.burnedOut }
    case "observer":
