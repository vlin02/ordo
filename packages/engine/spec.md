...
*Container reading* (not in scope): When rear input is container block, outputs SS proportional to fullness (0 = empty, 15 = full).
...
**Movement effects**:
- Entities on top: slipperiness 0.8 (vs 0.6 normal, 0.98 ice); walking speed ~1.36 m/s (~70% reduction)
- Entities landing on top: bounce proportional to fall velocity; no fall damage. Sneaking: no bounce, takes fall damage. Holding jump: normal jump height, no bounce, no fall damage.
- Entities in path of piston-pushed slime block: launched in push direction at ~20 m/s
